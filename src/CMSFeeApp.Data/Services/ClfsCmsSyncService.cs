...
/// <summary>
/// Downloads the CMS Clinical Laboratory Fee Schedule (CLFS) for a given year,
/// extracts it, parses it, and stores the data in the local database.
/// </summary>
public class ClfsCmsSyncService : ICmsSyncService
{
    private readonly HttpClient _httpClient;
    private readonly ClfsImportService _importService;
    ...
    [
        "https://www.cms.gov/files/zip/clfs{year2d}.zip",
        "https://www.cms.gov/files/zip/clfs{year}.zip",
        "https://www.cms.gov/files/zip/{year}-clinical-laboratory-fee-schedule.zip",
        "https://www.cms.gov/files/zip/clinical-laboratory-fee-schedule-{year}.zip",
        "https://www.cms.gov/files/zip/cy{year}-clfs.zip",
    ];

    private static readonly string[] SkipKeywords =
        ["addenda", "readme", "read_me", "guide", "instructions", "template"];

    public ClfsCmsSyncService(
        HttpClient httpClient,
        ClfsImportService importService,
    ...
    {
        progress?.Report($"Starting CMS CLFS sync for {year}…");
        ...
        try
        {
            ...
            if (zipBytes == null)
            {
                var msg = $"Could not find a CLFS fee schedule file for {year} at CMS.gov. " +
                          "Try importing manually using File → Import.";
                _importLogRepository.LogImport(FeeScheduleType.ClinicalLab, year, null, 0, false, msg);
            ...
            var tmpPath = Path.Combine(Path.GetTempPath(), $"cms_clfs_{year}_{Guid.NewGuid():N}.csv");
            try
            {
                ...
                if (fees.Count == 0)
                {
                    ...
                    _importLogRepository.LogImport(FeeScheduleType.ClinicalLab, year, dataFileName, 0, false, msg);
                    return new ImportResult { Success = false, ErrorMessage = msg };
                }
                ...
                await Task.Run(() =>
                {
                    _clfsRepository.DeleteFeesByYear(year, dataSource: "cms_download");
                    foreach (var f in fees) f.DataSource = "cms_download";
                    _clfsRepository.InsertFees(fees);
                }, ct);

                _importLogRepository.LogImport(FeeScheduleType.ClinicalLab, year, dataFileName, fees.Count, true);
                progress?.Report($"CMS CLFS sync complete: {fees.Count:N0} records imported for {year}.");
        ...
        {
            _importLogRepository.LogImport(FeeScheduleType.ClinicalLab, year, null, 0, false, ex.Message);
    ...
    private static (string fileName, byte[] data) ExtractMainCsvFromZip(byte[] zipBytes)
    {
        ...
        // Tier 1: contains "clfs" or "clinical", .csv, not skipped
        var tier1 = allNames.Where(n =>
            (n.ToLowerInvariant().Contains("clfs") || n.ToLowerInvariant().Contains("clinical")) &&
