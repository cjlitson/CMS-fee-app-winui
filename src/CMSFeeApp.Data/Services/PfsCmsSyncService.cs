...
/// <summary>
/// Downloads the CMS Physician Fee Schedule (PFS) national file for a given year,
...
/// </summary>
public class PfsCmsSyncService : ICmsSyncService
{
    private readonly HttpClient _httpClient;
    private readonly PfsImportService _importService;
    ...
    // CMS URL templates for PFS national fee schedule downloads.
    // {year} = 4-digit year; {year2d} = 2-digit year.
    private static readonly string[] UrlTemplates =
    [
        "https://www.cms.gov/files/zip/pfs{year2d}.zip",
        "https://www.cms.gov/files/zip/pfs{year}.zip",
        "https://www.cms.gov/files/zip/{year}-pfs-national-relative-value-file.zip",
        "https://www.cms.gov/files/zip/pfs-national-relative-value-file-{year}.zip",
        "https://www.cms.gov/files/zip/{year}-pfs-nrvf.zip",
        "https://www.cms.gov/files/zip/cy{year}-physician-fee-schedule.zip",
        "https://www.cms.gov/files/zip/cy{year2d}-physician-fee-schedule.zip",
    ];

    private static readonly string[] SkipKeywords =
        ["addenda", "readme", "read_me", "guide", "instructions", "template"];

    public PfsCmsSyncService(
        HttpClient httpClient,
        PfsImportService importService,
    ...
    {
        progress?.Report($"Starting CMS PFS sync for {year}…");
        ...
        try
        {
            ...
            if (zipBytes == null)
            {
                var msg = $"Could not find a PFS fee schedule file for {year} at CMS.gov. " +
                          "Try importing manually using File → Import.";
                _importLogRepository.LogImport(FeeScheduleType.PfsNational, year, null, 0, false, msg);
            ...
            var tmpPath = Path.Combine(Path.GetTempPath(), $"cms_pfs_{year}_{Guid.NewGuid():N}.csv");
            try
            {
                ...
                if (fees.Count == 0)
                {
                    ...
                    _importLogRepository.LogImport(FeeScheduleType.PfsNational, year, dataFileName, 0, false, msg);
                    return new ImportResult { Success = false, ErrorMessage = msg };
                }
                ...
                await Task.Run(() =>
                {
                    _pfsRepository.DeleteFeesByYear(year, dataSource: "cms_download");
                    foreach (var f in fees) f.DataSource = "cms_download";
                    _pfsRepository.InsertFees(fees);
                }, ct);

                _importLogRepository.LogImport(FeeScheduleType.PfsNational, year, dataFileName, fees.Count, true);
                progress?.Report($"CMS PFS sync complete: {fees.Count:N0} records imported for {year}.");
        ...
        {
            _importLogRepository.LogImport(FeeScheduleType.PfsNational, year, null, 0, false, ex.Message);
    ...
    private static (string fileName, byte[] data) ExtractMainCsvFromZip(byte[] zipBytes)
    {
        ...
        var entry = SelectMainPfsEntry(zip, allNames)
    ...
    private static ZipArchiveEntry? SelectMainPfsEntry(ZipArchive zip, List<string> allNames)
    {
        static bool IsSkipped(string name) =>
            SkipKeywords.Any(kw => name.ToLowerInvariant().Contains(kw));

        // Tier 1: contains "pfs" or "physician", .csv, not skipped
        var tier1 = allNames.Where(n =>
            (n.ToLowerInvariant().Contains("pfs") || n.ToLowerInvariant().Contains("physician")) &&
