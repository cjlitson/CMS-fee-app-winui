...
/// <summary>
/// Parses CMS Hospital Outpatient PPS (OPPS) Addendum B CSV files and imports
/// records into the database.
/// </summary>
public class OppsImportService : IFeeImporter
{
    private readonly OppsRepository _repository;
    ...
    public OppsImportService(OppsRepository repository, ImportLogRepository importLogRepository)
    {
    ...
    public async Task<ImportResult> ImportFromFileAsync(string filePath, int year, CancellationToken ct = default)
    {
        try
        {
            ...
            if (fees.Count == 0)
            {
                ...
                _importLogRepository.LogImport(FeeScheduleType.Opps, year, filePath, 0, false, msg);
                return new ImportResult { Success = false, RecordsImported = 0, ErrorMessage = msg };
            }

            await Task.Run(() => _repository.InsertFees(fees), ct);

            _importLogRepository.LogImport(FeeScheduleType.Opps, year, filePath, fees.Count, true);
            return new ImportResult { Success = true, RecordsImported = fees.Count };
        }
        catch (Exception ex)
        {
            _importLogRepository.LogImport(FeeScheduleType.Opps, year, filePath, 0, false, ex.Message);
            return new ImportResult { Success = false, RecordsImported = 0, ErrorMessage = ex.Message };
        }
    }

    internal IReadOnlyList<OppsFee> ParseFile(string filePath, int year)
    {
        var delimiter = DmeposImportService.DetectDelimiter(filePath);
        return ParseCsv(filePath, year, delimiter);
    }

    public IReadOnlyList<OppsFee> ParseCsv(string filePath, int year, char delimiter = ',')
    {
        var records = new List<OppsFee>();
        ...
        while ((line = stream.ReadLine()) != null)
        {
            ...
            var apcCode = (GetValue(row, "apc", "apc_code", "apc code") ?? "").Trim();
            if (string.IsNullOrEmpty(apcCode)) apcCode = null;

            var paymentRate = DmeposImportService.ParseAmount(
                GetValue(row, "payment rate", "payment_rate", "opps payment", "opps_payment", "rate"));

            var statusIndicator = (GetValue(row, "status indicator", "status_indicator", "si") ?? "").Trim();
            if (string.IsNullOrEmpty(statusIndicator)) statusIndicator = null;

            records.Add(new OppsFee
            {
                HcpcsCode = hcpcs,
    ...
    private static (int index, string? line) FindHeaderLine(StreamReader reader)
    {
        ...
        while ((line = reader.ReadLine()) != null)
        {
            ...
            if (lower.Contains("hcpcs") && (lower.Contains("description") || lower.Contains("apc") || lower.Contains("payment")))
                return (idx, line);
            idx++;
        }
        return (-1, null);
