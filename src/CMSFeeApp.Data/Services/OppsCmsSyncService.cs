...
/// <summary>
/// Downloads the CMS Hospital Outpatient PPS (OPPS) Addendum B file for a given year,
/// extracts it, parses it, and stores the data in the local database.
/// </summary>
public class OppsCmsSyncService : ICmsSyncService
{
    private readonly HttpClient _httpClient;
    private readonly OppsImportService _importService;
    private readonly OppsRepository _oppsRepository;
    private readonly ImportLogRepository _importLogRepository;

    public FeeScheduleType ScheduleType => FeeScheduleType.Opps;

    private static readonly string[] UrlTemplates =
    [
        "https://www.cms.gov/files/zip/opps{year2d}.zip",
        "https://www.cms.gov/files/zip/opps{year}.zip",
        "https://www.cms.gov/files/zip/{year}-opps-addendum-b.zip",
        "https://www.cms.gov/files/zip/cy{year}-opps-addendum-b.zip",
        "https://www.cms.gov/files/zip/{year}-hospital-outpatient-pps.zip",
        "https://www.cms.gov/files/zip/cy{year}-hospital-outpatient-pps.zip",
    ];

    private static readonly string[] SkipKeywords =
        ["addenda-a", "addendum-a", "readme", "read_me", "guide", "instructions", "template"];

    public OppsCmsSyncService(
        HttpClient httpClient,
        OppsImportService importService,
    ...
    {
        progress?.Report($"Starting CMS OPPS sync for {year}…");
        ...
        try
        {
            ...
            if (zipBytes == null)
            {
                var msg = $"Could not find an OPPS file for {year} at CMS.gov. " +
                          "Try importing manually using File → Import.";
                _importLogRepository.LogImport(FeeScheduleType.Opps, year, null, 0, false, msg);
            ...
            var tmpPath = Path.Combine(Path.GetTempPath(), $"cms_opps_{year}_{Guid.NewGuid():N}.csv");
            try
            {
                ...
                if (fees.Count == 0)
                {
                    ...
                    _importLogRepository.LogImport(FeeScheduleType.Opps, year, dataFileName, 0, false, msg);
                    return new ImportResult { Success = false, ErrorMessage = msg };
                }
                ...
                await Task.Run(() =>
                {
                    _oppsRepository.DeleteFeesByYear(year, dataSource: "cms_download");
                    foreach (var f in fees) f.DataSource = "cms_download";
                    _oppsRepository.InsertFees(fees);
                }, ct);

                _importLogRepository.LogImport(FeeScheduleType.Opps, year, dataFileName, fees.Count, true);
                progress?.Report($"CMS OPPS sync complete: {fees.Count:N0} records imported for {year}.");
        ...
        {
            _importLogRepository.LogImport(FeeScheduleType.Opps, year, null, 0, false, ex.Message);
    ...
    private static (string fileName, byte[] data) ExtractMainCsvFromZip(byte[] zipBytes)
    {
        ...
        // Tier 1: contains "addendum-b" or "addendumb" or "opps", .csv, not skipped
        var tier1 = allNames.Where(n =>
            (n.ToLowerInvariant().Contains("addendum-b") || n.ToLowerInvariant().Contains("addendumb") ||
             n.ToLowerInvariant().Contains("opps")) &&
