...
/// <summary>
/// Parses CMS Average Sales Price (ASP) drug pricing CSV files and imports
/// records into the database.
/// </summary>
public class AspImportService : IFeeImporter
{
    private readonly AspRepository _repository;
    ...
    public AspImportService(AspRepository repository, ImportLogRepository importLogRepository)
    {
    ...
    public async Task<ImportResult> ImportFromFileAsync(string filePath, int year, CancellationToken ct = default)
    {
        try
        {
            ...
            if (fees.Count == 0)
            {
                ...
                _importLogRepository.LogImport(FeeScheduleType.AspDrug, year, filePath, 0, false, msg);
                return new ImportResult { Success = false, RecordsImported = 0, ErrorMessage = msg };
            }

            await Task.Run(() => _repository.InsertFees(fees), ct);

            _importLogRepository.LogImport(FeeScheduleType.AspDrug, year, filePath, fees.Count, true);
            return new ImportResult { Success = true, RecordsImported = fees.Count };
        }
        catch (Exception ex)
        {
            _importLogRepository.LogImport(FeeScheduleType.AspDrug, year, filePath, 0, false, ex.Message);
            return new ImportResult { Success = false, RecordsImported = 0, ErrorMessage = ex.Message };
        }
    }

    internal IReadOnlyList<AspFee> ParseFile(string filePath, int year)
    {
        var delimiter = DmeposImportService.DetectDelimiter(filePath);
        return ParseCsv(filePath, year, delimiter);
    }

    public IReadOnlyList<AspFee> ParseCsv(string filePath, int year, char delimiter = ',')
    {
        var records = new List<AspFee>();
        ...
        while ((line = stream.ReadLine()) != null)
        {
            ...
            var description = GetValue(row, "description", "long_description", "short_description", "drug description") ?? "";

            var paymentLimit = DmeposImportService.ParseAmount(
                GetValue(row, "payment limit", "payment_limit", "asp payment limit", "asp_payment_limit", "limit", "price"));

            var dosageDescriptor = (GetValue(row, "dosage descriptor", "dosage_descriptor", "dosage") ?? "").Trim();
            if (string.IsNullOrEmpty(dosageDescriptor)) dosageDescriptor = null;

            // Try to find quarter from column or filename; default to 1 if not found
            int quarter = 1;
            var quarterRaw = GetValue(row, "quarter", "qtr");
            if (!string.IsNullOrWhiteSpace(quarterRaw) && int.TryParse(quarterRaw.Trim('Q', 'q'), out var q) && q >= 1 && q <= 4)
                quarter = q;

            records.Add(new AspFee
            {
                HcpcsCode = hcpcs,
                ...
                DosageDescriptor = dosageDescriptor,
    ...
    private static (int index, string? line) FindHeaderLine(StreamReader reader)
    {
        ...
        while ((line = reader.ReadLine()) != null)
        {
            ...
            if (lower.Contains("hcpcs") && (lower.Contains("description") || lower.Contains("payment") || lower.Contains("price")))
                return (idx, line);
            idx++;
        }
        return (-1, null);
